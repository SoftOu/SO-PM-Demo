...
namespace SOPasswordManager.Repo.DbEntities
{
    public partial class SOPasswordManagerContext : DbContext
    {
        ...
        //                //optionsBuilder.UseSqlServer("Server=tcp:sosrvr01.database.windows.net,1433;Database=SO-PM;user id=AdmJul;password=S*O*2018*team-AdmSrvr");
        //                //optionsBuilder.UseSqlServer("Server=localhost;Database=SO-PM;user id=usrpm;password=JXpb03U&Hz@Q");
        //                //optionsBuilder.UseSqlServer("Server=82.223.101.34;Database=SO-PM;user id=usrpm;password=JXpb03U&Hz@Q");
        //                optionsBuilder.UseSqlServer("Server=51.210.14.250:3306;Database=SO-PM;user id=usrpm;password=JXpb03U&Hz@Q");
        //            }
        //        }
        ...
        protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
        {
            /*
            optionsBuilder
                //.UseMySql("server=localhost;port=3306;user=root;password=sa1234;database=so-pm",
                
                //    //.UseMySql("server=localhost;port=3306;user=dbusername;password=dbpassword;database=dbname",
                //    b => b.ServerVersion(new ServerVersion("10.0.38-mariadb")))
                //.UseMySql("server=127.0.0.1;port=3306;user=root;password=sa1234;database=so-pm")
                //.UseMySql("server=localhost;port=3306;user=dbusername;password=dbpassword;database=dbname",)
            ...
            */
            ...
            //Funciona en local
            //string connectionString = "server=localhost;port=3306;database=SO-PM;user=root;password=";
            //Pero no en remoto. Buscar varios ejemplos con métodos diferentes
            //buscar si tiene que ver algo que no tenga https ni ssl

            //El problema por el que no funcionaba es porque en Plesk/Database Servers teníamos que activar la opción
            //Allow local MySQL server to accept external connections
            //en MariaDB
            string connectionString = "server=51.75.252.115;port=3306;database=SO-PM-Demo;user=userdemo;password=PassDemo#123";
